...
impl Default for TileOperation {
    fn default() -> Self {
        Self { m: 64, n: 64, k: 64, bytes_per_element: 2 }
    }
}
...
impl Default for AttentionOperation {
    fn default() -> Self {
        Self { seq_len_q: 1, seq_len_kv: 1024, head_dim: 128, num_queries: 32, kv_groups: 4, bytes_per_element: 2 }
    }
}
...
impl<'a> CycleEstimator<'a> {
    ...
    pub fn estimate_gemm_cycles(&self, op: &TileOperation) -> u64 {
        ...
        let bram_read_bw  = self.hw.bram.read_bandwidth_bytes_per_cycle as u64
                            * self.hw.bram.read_ports as u64;
    ...
    pub fn estimate_attention_cycles(&self, op: &AttentionOperation) -> u64 {
        ...
        let ops_qk = op.seq_len_q as u64 * op.seq_len_kv as u64 * op.head_dim as u64 * op.num_queries as u64;
        let ops_av = op.seq_len_q as u64 * op.head_dim as u64 * op.seq_len_kv as u64 * op.num_queries as u64;
        ...
        let bram_read_bw = self.hw.bram.read_bandwidth_bytes_per_cycle as u64 * self.hw.bram.read_ports as u64;
        let bram_write_bw = self.hw.bram.write_bandwidth_bytes_per_cycle as u64;
    ...
    pub fn arithmetic_intensity(&self, op: &TileOperation) -> f64 {
        ...
        if bytes_io == 0.0 { 0.0 } else { mac_ops / bytes_io }
    }
    ...
    pub fn is_compute_bound(&self, op: &TileOperation) -> bool {
        let peak_mac_intensity = {
            ...
            let bram_bw = self.hw.bram.read_bandwidth_bytes_per_cycle as f64
                * self.hw.bram.read_ports as f64;
