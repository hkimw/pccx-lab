...
def validate_mcp_sample_status_summary(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.mcp-sample-status-summary.v0")
    require_string_fields(
        root,
        "$",
        [
            "tool",
            "sampleStatusSummaryId",
            "summaryState",
            "sampleState",
            "resultState",
            "adapterState",
    ...
    expected_root_values = {
        ...
        "sampleState": "listed_not_executed",
        "resultState": "blocked_summary",
    ...
    if not any(
        ...
        raise ShapeError("missing mcp_sample_catalog disabled source reference")
    if not any(
        ref["refId"] == "mcp_sample_detail"
        and ref.get("sampleDetailAvailable") is True
        and ref.get("selectedSampleId") == "lab.status.read"
        and ref.get("sampleDiscoveryAllowed") is False
        and ref.get("resultPayloadReaderAllowed") is False
        and ref.get("toolInvocationAllowed") is False
        for ref in refs
    ):
        raise ShapeError("missing mcp_sample_detail disabled source reference")
    if not any(
        ref["refId"] == "mcp_tool_detail"
        and ref.get("toolDetailAvailable") is True
        and ref.get("selectedToolId") == "lab.status.read"
    ...
    summary = expect_object(require_field(root, "$", "summary"), "$.summary")
    require_string_fields(
        summary,
        "$.summary",
        [
    ...
    expect_integer(require_field(summary, "$.summary", "sampleCount"), "$.summary.sampleCount")
    expected_summary_values = {
        "selectedSampleId": "lab.status.read",
        "selectedToolId": "lab.status.read",
        "sampleCatalogState": "descriptor_only",
        "sampleDetailState": "descriptor_only",
        "samplePlanState": "descriptor_only",
        "sampleResultState": "blocked_summary",
        "sampleExecutionState": "not_executed",
        "toolInvocationState": "not_invoked",
        "permissionState": "descriptor_only",
        "overallState": "blocked",
    }
    for field, expected in expected_summary_values.items():
        if summary[field] != expected:
            raise ShapeError(f"unexpected value at $.summary.{field}: expected {expected}")
    summary_true_flags = [
        "summaryOnly",
        "descriptorOnly",
    ...
    require_bool_fields(summary, "$.summary", summary_true_flags + summary_false_flags)
    for flag in summary_true_flags:
        if summary[flag] is not True:
            raise ShapeError(f"unexpected value at $.summary.{flag}: expected true")
    for flag in summary_false_flags:
        if summary[flag] is not False:
            raise ShapeError(f"unexpected value at $.summary.{flag}: expected false")

    cards = require_object_array(require_field(root, "$", "statusCards"), "$.statusCards", min_items=1)
    card_ids = set()
    for card in cards:
        path = "$.statusCards[]"
        require_string_fields(
            card,
            path,
            ["cardId", "cardState", "severity", "sourceRef", "summary", "contentPolicy", "requiredBefore"],
        )
        ...
        for field in ["contentIncluded", "pathEchoAllowed", "rawCommandIncluded"]:
            if card[field] is not False:
                raise ShapeError(f"unexpected value at {child(path, field)}: expected false")
    for card_id in ["sample_catalog", "sample_detail", "sample_result", "execution_gate", "permission_gate"]:
        if card_id not in card_ids:
            raise ShapeError(f"missing MCP sample status card: {card_id}")
    ...
    if flow["flowState"] != "blocked":
        raise ShapeError("unexpected value at $.summaryFlow.flowState: expected blocked")
    if flow["commandKind"] != "planned-cli-fixed-args":
        raise ShapeError("unexpected value at $.summaryFlow.commandKind: expected planned-cli-fixed-args")
    require_string_array(
        require_field(flow, "$.summaryFlow", "fixedArgsPreview"),
        "$.summaryFlow.fixedArgsPreview",
        min_items=1,
    )
    steps = require_object_array(require_field(flow, "$.summaryFlow", "steps"), "$.summaryFlow.steps", min_items=1)
    step_ids = set()
    for step in steps:
        ...
        step_ids.add(step["stepId"])
    for step_id in [
        "sample_catalog_status",
        "sample_detail_status",
        "sample_result_status",
        "sample_invocation_gate",
    ]:
        if step_id not in step_ids:
            raise ShapeError(f"missing MCP sample status-summary flow step: {step_id}")
    require_string_array(
    ...
    review = expect_object(require_field(root, "$", "reviewGate"), "$.reviewGate")
    require_string_fields(review, "$.reviewGate", ["state", "summary"])
    review_true_flags = ["approvalRequiredBeforeImplementation", "approvedForSampleStatusSummaryDescriptor"]
    review_false_flags = [
        "approvedForSampleDiscovery",
        "approvedForClientImplementation",
        "approvedForTransport",
    ...
    true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "sampleStatusSummaryFixtureOnly",
    ]
    false_flags = [
        "sampleDiscoveryImplemented",
        "sampleToolImplemented",
...
def validate_plugin_sample_status_summary(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.plugin-sample-status-summary.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    if not any(
        ...
        raise ShapeError("missing plugin_sample_catalog disabled source reference")
    if not any(
        ref["refId"] == "plugin_sample_detail"
        and ref.get("sampleDetailAvailable") is True
        and ref.get("selectedSamplePluginId") == "example.diagnostics.summary"
        and ref.get("sampleDiscoveryAllowed") is False
        and ref.get("resultPayloadReaderAllowed") is False
        and ref.get("pluginInvocationAllowed") is False
        for ref in refs
    ):
        raise ShapeError("missing plugin_sample_detail disabled source reference")
    if not any(
        ref["refId"] == "plugin_capability_detail"
        and ref.get("capabilityDetailAvailable") is True
        and ref.get("capabilityId") == "plugin.diagnostics.summary"
    ...
    expect_integer(require_field(summary, "$.summary", "sampleCount"), "$.summary.sampleCount")
    expected_summary_values = {
        "selectedSamplePluginId": "example.diagnostics.summary",
        ...
        "sampleResultState": "blocked_summary",
        "sampleExecutionState": "not_executed",
        "pluginInvocationState": "not_invoked",
        "permissionState": "descriptor_only",
        "overallState": "blocked",
    }
    for field, expected in expected_summary_values.items():
    ...
    for card in cards:
        ...
        for field in ["contentIncluded", "pathEchoAllowed", "rawCommandIncluded"]:
            if card[field] is not False:
                raise ShapeError(f"unexpected value at {child(path, field)}: expected false")
    for card_id in ["sample_catalog", "sample_detail", "sample_result", "loader_gate", "permission_gate"]:
        if card_id not in card_ids:
            raise ShapeError(f"missing plugin sample status card: {card_id}")
    ...
    for step_id in [
        ...
        if step_id not in step_ids:
            raise ShapeError(f"missing plugin sample status-summary flow step: {step_id}")
    require_string_array(
        require_field(flow, "$.summaryFlow", "blockedReasonRefs"),
        "$.summaryFlow.blockedReasonRefs",
        min_items=1,
    ...
    review_false_flags = [
        ...
        "approvedForArtifactWrite",
        "approvedForRepositoryMutation",
        "approvedForPackageDistribution",
        "approvedForMarketplacePublication",
    ]
    require_bool_fields(review, "$.reviewGate", review_true_flags + review_false_flags)
    for flag in review_true_flags:
        if review[flag] is not True:
    ...
    true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "pluginSampleStatusSummaryFixtureOnly",
    ]
    false_flags = [
        "sampleDiscoveryImplemented",
        "samplePluginImplemented",
...
def validate_sail_interface_boundary(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-interface-boundary.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        "boundaryState": "descriptor_only",
        "sourceState": "not_read",
        "parserState": "not_implemented",
        "compilerState": "not_implemented",
        "modelState": "not_implemented",
        "refinementState": "blocked",
        "proofState": "blocked",
        "reportState": "summary_only",
    ...
    for ref in refs:
        require_string_fields(ref, "$.sourceBoundaryRefs[]", ["refId", "schemaVersion", "examplePath", "state"])
    if not any(
        ref["refId"] == "sail_adoption_plan"
        and ref.get("interfaceSource") is True
        and ref.get("sourceReadAllowed") is False
        and ref.get("executionAllowed") is False
        for ref in refs
    ):
        raise ShapeError("missing sail_adoption_plan interface source reference")
    if not any(
        ref["refId"] == "workflow_descriptors"
        and ref.get("summaryReferenceAllowed") is True
        and ref.get("commandExecutionAllowed") is False
        and ref.get("modelExecutionAllowed") is False
        for ref in refs
    ):
        raise ShapeError("missing workflow_descriptors Sail interface reference")
    if not any(
        ref["refId"] == "workflow_results"
        and ref.get("resultSummaryAvailable") is True
        and ref.get("payloadIncluded") is False
    ...
    contract = expect_object(require_field(root, "$", "interfaceContract"), "$.interfaceContract")
    require_string_fields(
        contract,
        "$.interfaceContract",
        ["contractKind", "scope", "issueRef", "handoffMode"],
    )
    contract_true_flags = ["summaryOnly", "descriptorOnly", "interfaceOnly", "cliCoreFirst", "guiSecond"]
    contract_false_flags = [
        "sailSourceReadAllowed",
        "parserRequestAllowed",
        "compilerRequestAllowed",
        "modelGenerationAllowed",
        "modelExecutionAllowed",
        "refinementExecutionAllowed",
        "formalProofAllowed",
        "reportReadAllowed",
        "reportWriteAllowed",
        "artifactReadAllowed",
    ...
    require_bool_fields(contract, "$.interfaceContract", contract_true_flags + contract_false_flags)
    for flag in contract_true_flags:
        if contract[flag] is not True:
            raise ShapeError(f"unexpected value at $.interfaceContract.{flag}: expected true")
    for flag in contract_false_flags:
        if contract[flag] is not False:
            raise ShapeError(f"unexpected value at $.interfaceContract.{flag}: expected false")

    endpoints = require_object_array(
        require_field(root, "$", "interfaceEndpoints"),
        "$.interfaceEndpoints",
        min_items=1,
    )
    endpoint_ids = set()
    ...
    for endpoint in endpoints:
        path = "$.interfaceEndpoints[]"
        require_string_fields(endpoint, path, ["endpointId", "state", "summary", "acceptedInputKind", "outputKind"])
        endpoint_ids.add(endpoint["endpointId"])
        if endpoint["state"] not in ["blocked", "descriptor_only"]:
            raise ShapeError("unexpected value at $.interfaceEndpoints[].state")
        require_bool_fields(endpoint, path, endpoint_false_flags)
        for flag in endpoint_false_flags:
            if endpoint[flag] is not False:
                raise ShapeError(f"unexpected value at $.interfaceEndpoints[].{flag}: expected false")
    for endpoint_id in [
        "sail_source_intake",
        "sail_parse_request",
        "sail_model_summary",
        ...
        if endpoint_id not in endpoint_ids:
            raise ShapeError(f"missing endpoint id in $.interfaceEndpoints: {endpoint_id}")
    ...
    safety = expect_object(require_field(root, "$", "safetyFlags"), "$.safetyFlags")
    true_flags = ["dataOnly", "descriptorOnly", "readOnly", "interfaceOnly", "summaryOnly", "cliCoreFirst", "guiSecond"]
    false_flags = [
        "sailSourceRead",
        "sailParserImplemented",
        "sailCompilerImplemented",
...
def validate_sail_review_packet(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-review-packet.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        "reviewState": "descriptor_only",
        "adapterState": "not_implemented",
        "defaultMode": "read_only",
        "packetKind": "summary_only_sail_review_packet",
        "automationPath": "cli_core_first",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")

    refs = require_object_array(require_field(root, "$", "sourceBoundaryRefs"), "$.sourceBoundaryRefs", min_items=5)
    ref_ids = set()
    ref_false_flags = [
        "sourceReadAllowed",
        "executionAllowed",
    ...
    for ref in refs:
        ...
        if ref.get("summaryReferenceAllowed") is not True:
            raise ShapeError("unexpected value at $.sourceBoundaryRefs[].summaryReferenceAllowed: expected true")
        for flag in ref_false_flags:
            if flag in ref and expect_bool(require_field(ref, path, flag), child(path, flag)) is not False:
    ...
    inputs = require_object_array(require_field(root, "$", "reviewInputs"), "$.reviewInputs", min_items=5)
    allowed_input_kinds = {
        "adoption_summary",
        "interface_summary",
        "workflow_summary",
    ...
    for item in inputs:
        path = "$.reviewInputs[]"
        require_string_fields(item, path, ["inputId", "inputKind", "inputState", "sourceRef", "summary"])
        input_id = item["inputId"]
        if input_id in input_ids:
            raise ShapeError(f"duplicate value at $.reviewInputs[].inputId: {input_id}")
        input_ids.add(input_id)
        if item["inputKind"] not in allowed_input_kinds:
            raise ShapeError(
                "unexpected value at $.reviewInputs[].inputKind: "
                f"expected one of {sorted(allowed_input_kinds)}"
            )
        if item["inputState"] != "approved_summary_only":
            raise ShapeError("unexpected value at $.reviewInputs[].inputState: expected approved_summary_only")
        require_bool_fields(item, path, ["summaryOnly", "approvalRequired"] + input_false_flags)
        for flag in ["summaryOnly", "approvalRequired"]:
            if item[flag] is not True:
                raise ShapeError(f"unexpected value at $.reviewInputs[].{flag}: expected true")
        for flag in input_false_flags:
            if item[flag] is not False:
                raise ShapeError(f"unexpected value at $.reviewInputs[].{flag}: expected false")
        fields = require_object_array(require_field(item, path, "fieldDescriptors"), child(path, "fieldDescriptors"), min_items=1)
        for field in fields:
            require_string_fields(field, "$.reviewInputs[].fieldDescriptors[]", ["fieldName", "valueKind", "policy"])

    policy = expect_object(require_field(root, "$", "reviewPolicy"), "$.reviewPolicy")
    require_string_fields(policy, "$.reviewPolicy", ["state", "packetKind", "redactionRule"])
    if policy["state"] != "planned":
        raise ShapeError("unexpected value at $.reviewPolicy.state: expected planned")
    if policy["packetKind"] != "summary_only_sail_review_packet":
        raise ShapeError("unexpected value at $.reviewPolicy.packetKind: expected summary_only_sail_review_packet")
    expect_integer(require_field(policy, "$.reviewPolicy", "maxSections"), "$.reviewPolicy.maxSections")
    policy_true_flags = ["summaryOnly", "approvalRequired", "auditRequired"]
    policy_false_flags = [
        "sailSourceReadAllowed",
        "rtlSourceReadAllowed",
        "sailAstReadAllowed",
    ...
    require_bool_fields(policy, "$.reviewPolicy", policy_true_flags + policy_false_flags)
    ...
    packet = expect_object(require_field(root, "$", "sampleReviewPacket"), "$.sampleReviewPacket")
    require_string_fields(packet, "$.sampleReviewPacket", ["reviewPacketState", "targetRepository", "titlePreview"])
    if packet["reviewPacketState"] != "summary_only_fixture":
        raise ShapeError("unexpected value at $.sampleReviewPacket.reviewPacketState: expected summary_only_fixture")
    section_false_flags = [
        "pathIncluded",
    ...
    sections = require_object_array(require_field(packet, "$.sampleReviewPacket", "reviewSections"), "$.sampleReviewPacket.reviewSections", min_items=2)
    for section in sections:
        path = "$.sampleReviewPacket.reviewSections[]"
        require_string_fields(section, path, ["sectionId", "sectionKind", "heading", "summary"])
        require_bool_fields(section, path, ["summaryOnly"] + section_false_flags)
        if section["summaryOnly"] is not True:
            raise ShapeError("unexpected value at $.sampleReviewPacket.reviewSections[].summaryOnly: expected true")
        for flag in section_false_flags:
            if section[flag] is not False:
                raise ShapeError(f"unexpected value at $.sampleReviewPacket.reviewSections[].{flag}: expected false")

    for array_name, min_items in [("riskRows", 1), ("checklistItems", 1), ("validationLines", 1)]:
        rows = require_object_array(require_field(packet, "$.sampleReviewPacket", array_name), child("$.sampleReviewPacket", array_name), min_items=min_items)
        for row in rows:
            path = f"$.sampleReviewPacket.{array_name}[]"
            require_bool_fields(row, path, ["summaryOnly"])
            if row["summaryOnly"] is not True:
                raise ShapeError(f"unexpected value at {path}.summaryOnly: expected true")
            if array_name == "validationLines":
                require_string_fields(row, path, ["lineId", "result", "summary"])
                false_flags = ["commandIncluded", "stdoutIncluded", "stderrIncluded", "rawLogsIncluded", "artifactPathIncluded"]
            elif array_name == "checklistItems":
                require_string_fields(row, path, ["itemId", "state", "label"])
                false_flags = ["commandIncluded", "pathIncluded", "artifactPathIncluded"]
            else:
                require_string_fields(row, path, ["rowId", "severity", "sourceRef", "summary", "mitigation"])
                false_flags = ["commandIncluded", "pathIncluded", "artifactPathIncluded"]
            require_bool_fields(row, path, false_flags)
            for flag in false_flags:
                if row[flag] is not False:
                    raise ShapeError(f"unexpected value at {path}.{flag}: expected false")

    packet_true_flags = ["summaryOnly"]
    packet_false_flags = section_false_flags + ["reviewOutputPublished", "projectUpdated"]
    require_bool_fields(packet, "$.sampleReviewPacket", packet_true_flags + packet_false_flags)
    if packet["summaryOnly"] is not True:
        raise ShapeError("unexpected value at $.sampleReviewPacket.summaryOnly: expected true")
    for flag in packet_false_flags:
        if packet[flag] is not False:
            raise ShapeError(f"unexpected value at $.sampleReviewPacket.{flag}: expected false")
    ...
    safety = expect_object(require_field(root, "$", "safetyFlags"), "$.safetyFlags")
    safety_true_flags = ["dataOnly", "descriptorOnly", "readOnly", "sailReviewPacketFixtureOnly", "summaryOnly", "cliCoreFirst"]
    safety_false_flags = [
        "sailSourceRead",
        "rtlSourceRead",
        "sailAstIncluded",
...
def validate_sail_evidence_manifest(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-evidence-manifest.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        "manifestState": "descriptor_only",
        "adapterState": "not_implemented",
        "defaultMode": "read_only",
        "manifestKind": "approved_sail_summary_evidence_refs",
        "automationPath": "cli_core_first",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
    ...
    for ref in refs:
        ...
        if ref.get("summaryReferenceAllowed") is not True:
            raise ShapeError("unexpected value at $.sourceBoundaryRefs[].summaryReferenceAllowed: expected true")
        for flag in ref_false_flags:
            if expect_bool(require_field(ref, path, flag), child(path, flag)) is not False:
                raise ShapeError(f"unexpected value at $.sourceBoundaryRefs[].{flag}: expected false")
    for ref_id in [
        "sail_adoption_plan",
        "sail_interface_boundary",
    ...
    evidence_refs = require_object_array(require_field(root, "$", "approvedEvidenceRefs"), "$.approvedEvidenceRefs", min_items=5)
    ...
    for evidence in evidence_refs:
        path = "$.approvedEvidenceRefs[]"
        require_string_fields(evidence, path, ["evidenceId", "evidenceKind", "evidenceState", "sourceRef", "summary"])
        evidence_id = evidence["evidenceId"]
        if evidence_id in evidence_ids:
            raise ShapeError(f"duplicate value at $.approvedEvidenceRefs[].evidenceId: {evidence_id}")
        evidence_ids.add(evidence_id)
        if evidence["evidenceKind"] not in allowed_evidence_kinds:
            raise ShapeError(
                "unexpected value at $.approvedEvidenceRefs[].evidenceKind: "
                f"expected one of {sorted(allowed_evidence_kinds)}"
            )
        if evidence["evidenceState"] != "approved_summary_only":
            raise ShapeError("unexpected value at $.approvedEvidenceRefs[].evidenceState: expected approved_summary_only")
        require_bool_fields(evidence, path, ["summaryOnly", "approvalRequired", "approvedSummaryRef"] + evidence_false_flags)
        for flag in ["summaryOnly", "approvalRequired", "approvedSummaryRef"]:
            if evidence[flag] is not True:
                raise ShapeError(f"unexpected value at $.approvedEvidenceRefs[].{flag}: expected true")
        for flag in evidence_false_flags:
            if evidence[flag] is not False:
                raise ShapeError(f"unexpected value at $.approvedEvidenceRefs[].{flag}: expected false")
        fields = require_object_array(require_field(evidence, path, "fieldDescriptors"), child(path, "fieldDescriptors"), min_items=1)
        for field in fields:
            require_string_fields(field, "$.approvedEvidenceRefs[].fieldDescriptors[]", ["fieldName", "valueKind", "policy"])

    policy = expect_object(require_field(root, "$", "manifestPolicy"), "$.manifestPolicy")
    require_string_fields(policy, "$.manifestPolicy", ["state", "manifestKind", "redactionRule"])
    if policy["state"] != "planned":
        raise ShapeError("unexpected value at $.manifestPolicy.state: expected planned")
    if policy["manifestKind"] != "summary_only_sail_evidence_refs":
        raise ShapeError("unexpected value at $.manifestPolicy.manifestKind: expected summary_only_sail_evidence_refs")
    expect_integer(require_field(policy, "$.manifestPolicy", "maxEvidenceRefs"), "$.manifestPolicy.maxEvidenceRefs")
    policy_true_flags = ["summaryOnly", "approvalRequired", "auditRequired"]
    policy_false_flags = [
        "sailSourceReadAllowed",
    ...
    require_bool_fields(policy, "$.manifestPolicy", policy_true_flags + policy_false_flags)
    for flag in policy_true_flags:
        if policy[flag] is not True:
            raise ShapeError(f"unexpected value at $.manifestPolicy.{flag}: expected true")
    for flag in policy_false_flags:
    ...
    require_string_fields(manifest, "$.sampleManifest", ["manifestState", "targetScope", "summary"])
    if manifest["manifestState"] != "summary_only_fixture":
        raise ShapeError("unexpected value at $.sampleManifest.manifestState: expected summary_only_fixture")
    manifest_true_flags = ["summaryOnly"]
    ...
    require_bool_fields(manifest, "$.sampleManifest", manifest_true_flags + manifest_false_flags)
    if manifest["summaryOnly"] is not True:
        raise ShapeError("unexpected value at $.sampleManifest.summaryOnly: expected true")
    for flag in manifest_false_flags:
        if manifest[flag] is not False:
            raise ShapeError(f"unexpected value at $.sampleManifest.{flag}: expected false")
    row_false_flags = [
        "pathIncluded",
        "privatePathsIncluded",
        "stdoutIncluded",
    ...
    rows = require_object_array(require_field(manifest, "$.sampleManifest", "evidenceRows"), "$.sampleManifest.evidenceRows", min_items=3)
    for row in rows:
        path = "$.sampleManifest.evidenceRows[]"
        require_string_fields(row, path, ["rowId", "evidenceKind", "sourceRef", "state", "summary"])
        require_bool_fields(row, path, ["summaryOnly"] + row_false_flags)
        if row["summaryOnly"] is not True:
            raise ShapeError("unexpected value at $.sampleManifest.evidenceRows[].summaryOnly: expected true")
        for flag in row_false_flags:
            if row[flag] is not False:
                raise ShapeError(f"unexpected value at $.sampleManifest.evidenceRows[].{flag}: expected false")
    ...
    safety = expect_object(require_field(root, "$", "safetyFlags"), "$.safetyFlags")
    safety_true_flags = ["dataOnly", "descriptorOnly", "readOnly", "sailEvidenceManifestFixtureOnly", "summaryOnly", "cliCoreFirst"]
    safety_false_flags = [
        "sailSourceRead",
        "rtlSourceRead",
        "sailAstIncluded",
...
def validate_sail_evidence_detail(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-evidence-detail.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    for ref in refs:
        require_string_fields(ref, "$.sourceBoundaryRefs[]", ["refId", "schemaVersion", "examplePath", "state"])
    if not any(
        ref["refId"] == "sail_evidence_manifest"
        and ref.get("evidenceManifestAvailable") is True
        and ref.get("evidenceDetailAvailable") is True
        and ref.get("summaryOnly") is True
        and ref.get("sourceReaderAllowed") is False
        and ref.get("rtlSourceReaderAllowed") is False
        and ref.get("astReaderAllowed") is False
        and ref.get("generatedModelReaderAllowed") is False
        and ref.get("artifactReaderAllowed") is False
        and ref.get("reportReaderAllowed") is False
        and ref.get("reportWriterAllowed") is False
        and ref.get("commandExecutionAllowed") is False
        ...
        raise ShapeError("missing sail_evidence_manifest disabled source reference")
    if not any(
        ref["refId"] == "sail_review_packet"
        and ref.get("summaryOnly") is True
        and ref.get("sourceReadAllowed") is False
        and ref.get("rtlSourceReadAllowed") is False
        and ref.get("astReadAllowed") is False
        and ref.get("generatedModelReadAllowed") is False
        and ref.get("commandExecutionAllowed") is False
        and ref.get("repositoryMutationAllowed") is False
        for ref in refs
    ):
        raise ShapeError("missing sail_review_packet summary-only source reference")
    if not any(
        ref["refId"] == "sail_interface_boundary"
        and ref.get("summaryOnly") is True
        and ref.get("sourceReadAllowed") is False
        and ref.get("rtlSourceReadAllowed") is False
        and ref.get("astReadAllowed") is False
        ...
        raise ShapeError("missing sail_interface_boundary descriptor source reference")
    ...
    if request["requestKind"] != "planned_sail_evidence_detail":
        raise ShapeError("unexpected value at $.evidenceDetailRequest.requestKind: expected planned_sail_evidence_detail")
    if request["selectedEvidenceId"] != "sail_adoption_summary_evidence":
        raise ShapeError(
            "unexpected value at $.evidenceDetailRequest.selectedEvidenceId: "
            "expected sail_adoption_summary_evidence"
        )
    require_string_array(
        require_field(request, "$.evidenceDetailRequest", "fixedArgsPreview"),
        "$.evidenceDetailRequest.fixedArgsPreview",
    ...
    request_false_flags = [
        ...
        "modelExecutionReadAllowed",
        "refinementResultReadAllowed",
        "proofResultReadAllowed",
    ...
    output_false_flags = [
        ...
        "modelExecutionIncluded",
        "refinementResultIncluded",
        "proofResultIncluded",
        "rawReportIncluded",
        "hardwareDumpIncluded",
        "boardDumpIncluded",
    ...
    access_false_flags = [
        ...
        "approvedForReportRead",
        "approvedForSourceRead",
        "approvedForRtlSourceRead",
        "approvedForAstRead",
        "approvedForGeneratedModelRead",
        "approvedForModelExecution",
        "approvedForRefinementCheck",
        "approvedForFormalProof",
        "localFileReadAllowed",
        "repositoryReadAllowed",
        "sailSourceReadAllowed",
        "rtlSourceReadAllowed",
    ...
    related = require_object_array(require_field(root, "$", "relatedEvidenceRefs"), "$.relatedEvidenceRefs", min_items=1)
    for evidence in related:
        path = "$.relatedEvidenceRefs[]"
        require_string_fields(evidence, path, ["evidenceId", "relationship"])
        related_false_flags = [
        ...
        require_bool_fields(evidence, path, related_false_flags)
        for flag in related_false_flags:
            if evidence[flag] is not False:
                raise ShapeError(f"unexpected value at $.relatedEvidenceRefs[].{flag}: expected false")
    ...
    safety = expect_object(require_field(root, "$", "safetyFlags"), "$.safetyFlags")
    safety_true_flags = ["dataOnly", "descriptorOnly", "readOnly", "summaryOnly", "sailEvidenceDetailFixtureOnly"]
    safety_false_flags = [
...
def validate_sail_model_readiness(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-model-readiness.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        "readinessState": "descriptor_only",
        "adapterState": "not_implemented",
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "gateKind": "future_sail_model_readiness_gate",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    for ref in refs:
        ...
        if ref.get("summaryOnly") is not True:
            raise ShapeError("unexpected value at $.sourceBoundaryRefs[].summaryOnly: expected true")
        if ref.get("readinessInput") is not True:
            raise ShapeError("unexpected value at $.sourceBoundaryRefs[].readinessInput: expected true")
        for flag in [
            "sourceReadAllowed",
            "rtlSourceReadAllowed",
            "astReadAllowed",
            ...
            "repositoryMutationAllowed",
            "sourceReaderAllowed",
            "rtlSourceReaderAllowed",
            "astReaderAllowed",
            "generatedModelReaderAllowed",
            "artifactReaderAllowed",
            "reportReaderAllowed",
        ]:
    ...
    request = expect_object(require_field(root, "$", "readinessRequest"), "$.readinessRequest")
    require_string_fields(
        request,
        "$.readinessRequest",
        [
    ...
    if request["requestKind"] != "planned_sail_model_readiness":
        raise ShapeError("unexpected value at $.readinessRequest.requestKind: expected planned_sail_model_readiness")
    if request["outputBoundary"] != "pccx.lab.sail-model-readiness.v0":
        raise ShapeError("unexpected value at $.readinessRequest.outputBoundary")
    fixed_args = require_field(request, "$.readinessRequest", "fixedArgsPreview")
    require_string_array(fixed_args, "$.readinessRequest.fixedArgsPreview", min_items=4)
    if fixed_args[:4] != ["sail-model", "readiness", "--format", "json"]:
        raise ShapeError("unexpected value at $.readinessRequest.fixedArgsPreview")
    request_true_flags = ["summaryOnly", "inputRefOnly"]
    request_false_flags = [
        "approvalRequired",
        "pathEchoAllowed",
    ...
    require_bool_fields(request, "$.readinessRequest", request_true_flags + request_false_flags)
    for flag in request_true_flags:
        if request[flag] is not True:
            raise ShapeError(f"unexpected value at $.readinessRequest.{flag}: expected true")
    for flag in request_false_flags:
        if request[flag] is not False:
            raise ShapeError(f"unexpected value at $.readinessRequest.{flag}: expected false")

    checklist = require_object_array(
        require_field(root, "$", "readinessChecklist"),
        "$.readinessChecklist",
        min_items=3,
    )
    checklist_ids = set()
    checklist_true_flags = [
        "required",
        "summaryAvailable",
        "summaryOnly",
        "requiresSeparateSourceBoundary",
        "requiresSeparateParserBoundary",
        "requiresSeparateExecutionBoundary",
    ]
    ...
    for item in checklist:
        ...
        require_string_fields(
            item,
            path,
            ["checkId", "sourceRef", "readinessContribution", "checkState"],
        )
        checklist_ids.add(item["checkId"])
        if item["checkState"] != "metadata_ready":
            raise ShapeError("unexpected value at $.readinessChecklist[].checkState: expected metadata_ready")
        require_bool_fields(item, path, checklist_true_flags + checklist_false_flags)
        for flag in checklist_true_flags:
            if item[flag] is not True:
                raise ShapeError(f"unexpected value at $.readinessChecklist[].{flag}: expected true")
        for flag in checklist_false_flags:
            if item[flag] is not False:
                raise ShapeError(f"unexpected value at $.readinessChecklist[].{flag}: expected false")
    for check_id in [
        ...
        if check_id not in checklist_ids:
            raise ShapeError(f"missing readiness checklist item: {check_id}")

    result = expect_object(require_field(root, "$", "readinessResult"), "$.readinessResult")
    require_string_fields(
        result,
        "$.readinessResult",
        ["resultState", "modelReadiness", "cliCoreGate", "guiGate", "summary"],
    )
    if result["resultState"] != "metadata_only":
        raise ShapeError("unexpected value at $.readinessResult.resultState: expected metadata_only")
    if result["modelReadiness"] != "not_ready_for_execution":
        raise ShapeError("unexpected value at $.readinessResult.modelReadiness: expected not_ready_for_execution")
    result_true_flags = ["summaryOnly"]
    result_false_flags = [
    ...
    require_bool_fields(result, "$.readinessResult", result_true_flags + result_false_flags)
    if result["summaryOnly"] is not True:
        raise ShapeError("unexpected value at $.readinessResult.summaryOnly: expected true")
    for flag in result_false_flags:
        if result[flag] is not False:
            raise ShapeError(f"unexpected value at $.readinessResult.{flag}: expected false")
    ...
    safety = expect_object(require_field(root, "$", "safetyFlags"), "$.safetyFlags")
    safety_true_flags = ["dataOnly", "descriptorOnly", "readOnly", "summaryOnly", "sailModelReadinessFixtureOnly"]
    safety_false_flags = [
        "sailSourceReaderImplemented",
        "rtlSourceReaderImplemented",
        "sailAstReaderImplemented",
...
def validate_sail_implementation_gap_matrix(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-implementation-gap-matrix.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        "matrixState": "descriptor_only",
        "adapterState": "not_implemented",
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "matrixKind": "future_sail_implementation_gap_matrix",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    for ref in refs:
        ...
        if ref.get("summaryOnly") is not True:
            raise ShapeError("unexpected value at $.sourceBoundaryRefs[].summaryOnly: expected true")
        if ref.get("gapInput") is not True:
            raise ShapeError("unexpected value at $.sourceBoundaryRefs[].gapInput: expected true")
        for flag in [
            "sourceReadAllowed",
            "rtlSourceReadAllowed",
            "astReadAllowed",
    ...
    request = expect_object(require_field(root, "$", "gapRequest"), "$.gapRequest")
    require_string_fields(
        request,
        "$.gapRequest",
        [
    ...
    if request["requestKind"] != "planned_sail_implementation_gap_matrix":
        raise ShapeError("unexpected value at $.gapRequest.requestKind")
    if request["outputBoundary"] != "pccx.lab.sail-implementation-gap-matrix.v0":
        raise ShapeError("unexpected value at $.gapRequest.outputBoundary")
    fixed_args = require_field(request, "$.gapRequest", "fixedArgsPreview")
    require_string_array(fixed_args, "$.gapRequest.fixedArgsPreview", min_items=4)
    if fixed_args[:4] != ["sail-model", "gap-matrix", "--format", "json"]:
        raise ShapeError("unexpected value at $.gapRequest.fixedArgsPreview")
    request_true_flags = ["summaryOnly", "inputRefOnly"]
    request_false_flags = [
        "approvalRequired",
        "pathEchoAllowed",
    ...
    require_bool_fields(request, "$.gapRequest", request_true_flags + request_false_flags)
    for flag in request_true_flags:
        if request[flag] is not True:
            raise ShapeError(f"unexpected value at $.gapRequest.{flag}: expected true")
    for flag in request_false_flags:
        if request[flag] is not False:
            raise ShapeError(f"unexpected value at $.gapRequest.{flag}: expected false")

    rows = require_object_array(require_field(root, "$", "gapRows"), "$.gapRows", min_items=5)
    row_ids = set()
    row_true_flags = ["summaryOnly", "requiredBeforeImplementation", "separateBoundaryRequired"]
    row_false_flags = [
        "sourceReadAllowed",
        "rtlSourceReadAllowed",
        "astReadAllowed",
    ...
    for row in rows:
        path = "$.gapRows[]"
        require_string_fields(row, path, ["gapId", "gapKind", "gapState", "blockedUntil", "summary"])
        row_ids.add(row["gapId"])
        if row["gapState"] != "blocked_pending_separate_boundary":
            raise ShapeError("unexpected value at $.gapRows[].gapState")
        source_refs = require_field(row, path, "sourceRefs")
        require_string_array(source_refs, "$.gapRows[].sourceRefs", min_items=1)
        require_bool_fields(row, path, row_true_flags + row_false_flags)
        for flag in row_true_flags:
            if row[flag] is not True:
                raise ShapeError(f"unexpected value at $.gapRows[].{flag}: expected true")
        for flag in row_false_flags:
            if row[flag] is not False:
                raise ShapeError(f"unexpected value at $.gapRows[].{flag}: expected false")
    for row_id in [
        "source_boundary_gap",
        ...
        if row_id not in row_ids:
            raise ShapeError(f"missing gap row: {row_id}")

    summary = expect_object(require_field(root, "$", "gapSummary"), "$.gapSummary")
    require_string_fields(
        summary,
        "$.gapSummary",
        ["summaryState", "implementationState", "gapDisposition", "summary"],
    )
    expected_summary = {
        "summaryState": "metadata_only",
        "implementationState": "not_implemented",
        "gapDisposition": "blocked_by_open_gaps",
    }
    for field, expected in expected_summary.items():
        if summary[field] != expected:
            raise ShapeError(f"unexpected value at $.gapSummary.{field}: expected {expected}")
    summary_true_flags = ["summaryOnly"]
    summary_false_flags = [
    ...
    require_bool_fields(summary, "$.gapSummary", summary_true_flags + summary_false_flags)
    if summary["summaryOnly"] is not True:
        raise ShapeError("unexpected value at $.gapSummary.summaryOnly: expected true")
    for flag in summary_false_flags:
        if summary[flag] is not False:
            raise ShapeError(f"unexpected value at $.gapSummary.{flag}: expected false")
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "sailImplementationGapMatrixFixtureOnly",
    ]
    safety_false_flags = [
        "sailSourceReaderImplemented",
        "rtlSourceReaderImplemented",
...
def validate_sail_source_intake_approval(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-source-intake-approval.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "boundaryKind": "future_sail_source_intake_approval_gate",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    for ref in refs:
        ...
        require_string_fields(ref, path, ["refId", "schemaVersion", "examplePath", "state"])
        ref_ids.add(ref["refId"])
        for flag in ["summaryOnly", "approvalInput", "approvedSummaryRef"]:
            if ref.get(flag) is not True:
    ...
    request = expect_object(require_field(root, "$", "approvalRequest"), "$.approvalRequest")
    require_string_fields(
        request,
        "$.approvalRequest",
        [
    ...
    if request["requestKind"] != "planned_sail_source_intake_approval":
        raise ShapeError("unexpected value at $.approvalRequest.requestKind")
    if request["outputBoundary"] != "pccx.lab.sail-source-intake-approval.v0":
        raise ShapeError("unexpected value at $.approvalRequest.outputBoundary")
    fixed_args = require_field(request, "$.approvalRequest", "fixedArgsPreview")
    require_string_array(fixed_args, "$.approvalRequest.fixedArgsPreview", min_items=5)
    if fixed_args[:5] != ["sail-model", "source-intake", "approval", "--format", "json"]:
        raise ShapeError("unexpected value at $.approvalRequest.fixedArgsPreview")
    ...
    require_bool_fields(request, "$.approvalRequest", request_true_flags + request_false_flags)
    for flag in request_true_flags:
        if request[flag] is not True:
            raise ShapeError(f"unexpected value at $.approvalRequest.{flag}: expected true")
    for flag in request_false_flags:
        if request[flag] is not False:
            raise ShapeError(f"unexpected value at $.approvalRequest.{flag}: expected false")

    decision = expect_object(require_field(root, "$", "approvalDecisionGate"), "$.approvalDecisionGate")
    require_string_fields(
        decision,
        "$.approvalDecisionGate",
        ["gateState", "decisionState", "requestState", "disposition", "summary"],
    )
    expected_decision = {
        "gateState": "blocked",
        "decisionState": "not_approved",
        "requestState": "not_requested",
        "disposition": "blocked_until_explicit_source_intake_approval",
    }
    for field, expected in expected_decision.items():
        if decision[field] != expected:
            raise ShapeError(f"unexpected value at $.approvalDecisionGate.{field}: expected {expected}")
    decision_true_flags = ["summaryOnly", "descriptorOnly", "approvalRequired"]
    decision_false_flags = [
        "approvalRequested",
        "approvalRecorded",
        "approved",
    ...
    require_bool_fields(decision, "$.approvalDecisionGate", decision_true_flags + decision_false_flags)
    for flag in decision_true_flags:
        if decision[flag] is not True:
            raise ShapeError(f"unexpected value at $.approvalDecisionGate.{flag}: expected true")
    for flag in decision_false_flags:
        if decision[flag] is not False:
            raise ShapeError(f"unexpected value at $.approvalDecisionGate.{flag}: expected false")
    ...
    for item in checklist:
        path = "$.approvalChecklist[]"
        require_string_fields(item, path, ["checkId", "state", "sourceRef", "summary"])
        require_bool_fields(item, path, checklist_true_flags + checklist_false_flags)
        for flag in checklist_true_flags:
            if item[flag] is not True:
                raise ShapeError(f"unexpected value at $.approvalChecklist[].{flag}: expected true")
        for flag in checklist_false_flags:
            if item[flag] is not False:
                raise ShapeError(f"unexpected value at $.approvalChecklist[].{flag}: expected false")
    expected_checks = {
        "approved_source_intake_boundary_summary": "summary_available",
        "explicit_source_intake_approval": "not_requested",
        "no_source_material_boundary": "blocked",
    }
    for check_id, state in expected_checks.items():
        if not any(item["checkId"] == check_id and item["state"] == state for item in checklist):
            raise ShapeError(f"missing approval checklist item: {check_id}")

    policy = expect_object(require_field(root, "$", "intakeApprovalPolicy"), "$.intakeApprovalPolicy")
    require_string_fields(policy, "$.intakeApprovalPolicy", ["policyState", "requestState", "approvalState", "summary"])
    expected_policy = {
        "policyState": "blocked",
        "requestState": "not_requested",
        "approvalState": "not_approved",
    ...
    for field, expected in expected_policy.items():
        if policy[field] != expected:
            raise ShapeError(f"unexpected value at $.intakeApprovalPolicy.{field}: expected {expected}")
    ...
    require_bool_fields(policy, "$.intakeApprovalPolicy", policy_true_flags + policy_false_flags)
    for flag in policy_true_flags:
        if policy[flag] is not True:
            raise ShapeError(f"unexpected value at $.intakeApprovalPolicy.{flag}: expected true")
    for flag in policy_false_flags:
        if policy[flag] is not False:
            raise ShapeError(f"unexpected value at $.intakeApprovalPolicy.{flag}: expected false")
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "sailSourceIntakeApprovalFixtureOnly",
    ]
    safety_false_flags = [
        "approvalExecutorImplemented",
...
def validate_hybrid_source_intake_approval(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-source-intake-approval.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "boundaryKind": "future_hybrid_source_intake_approval_gate",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    if request["requestKind"] != "planned_hybrid_source_intake_approval":
        raise ShapeError("unexpected value at $.approvalRequest.requestKind")
    if request["sourceReferenceKind"] != "approved-hybrid-source-intake-summaries":
        raise ShapeError("unexpected value at $.approvalRequest.sourceReferenceKind")
    if request["outputBoundary"] != "pccx.lab.hybrid-source-intake-approval.v0":
        raise ShapeError("unexpected value at $.approvalRequest.outputBoundary")
    fixed_args = require_field(request, "$.approvalRequest", "fixedArgsPreview")
    require_string_array(fixed_args, "$.approvalRequest.fixedArgsPreview", min_items=5)
    if fixed_args[:5] != ["hybrid", "source-intake", "approval", "--format", "json"]:
        raise ShapeError("unexpected value at $.approvalRequest.fixedArgsPreview")
    request_true_flags = ["summaryOnly", "inputRefOnly", "approvalRequired"]
    request_false_flags = [
        "approvalRequested",
    ...
    expected_decision = {
        ...
        "decisionState": "not_approved",
        "requestState": "not_requested",
        "disposition": "blocked_until_explicit_hybrid_source_intake_approval",
    }
    for field, expected in expected_decision.items():
        if decision[field] != expected:
            raise ShapeError(f"unexpected value at $.approvalDecisionGate.{field}: expected {expected}")
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "hybridSourceIntakeApprovalFixtureOnly",
    ]
    safety_false_flags = [
        "approvalExecutorImplemented",
        "sourceIntakeApprovalImplemented",
...
def validate_hybrid_source_intake_result(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-source-intake-result.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "boundaryKind": "future_hybrid_source_intake_result_boundary",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    for ref in refs:
        ...
        require_string_fields(ref, path, ["refId", "schemaVersion", "examplePath", "state"])
        ref_ids.add(ref["refId"])
        for flag in ["summaryOnly", "resultInput", "approvedSummaryRef"]:
            if ref.get(flag) is not True:
                raise ShapeError(f"unexpected value at $.sourceBoundaryRefs[].{flag}: expected true")
        for flag in [
            "sourceIntakeAllowed",
    ...
    for ref_id in [
        ...
        if ref_id not in ref_ids:
            raise ShapeError(f"missing source boundary ref: {ref_id}")
    if not any(ref["refId"] == "hybrid_source_intake_approval" and ref.get("approved") is False for ref in refs):
        raise ShapeError("missing not-approved hybrid source-intake approval reference")
    if not any(
        ref["refId"] == "hybrid_implementation_gap_matrix" and ref.get("sourceBoundaryGapRecorded") is True
        for ref in refs
    ):
    ...
    request = expect_object(require_field(root, "$", "resultRequest"), "$.resultRequest")
    require_string_fields(
        request,
        "$.resultRequest",
        ["requestKind", "commandKind", "sourceReferenceKind", "outputBoundary", "summary"],
    )
    if request["requestKind"] != "planned_hybrid_source_intake_result":
        raise ShapeError("unexpected value at $.resultRequest.requestKind")
    if request["sourceReferenceKind"] != "approved-hybrid-source-intake-result-summaries":
        raise ShapeError("unexpected value at $.resultRequest.sourceReferenceKind")
    if request["outputBoundary"] != "pccx.lab.hybrid-source-intake-result.v0":
        raise ShapeError("unexpected value at $.resultRequest.outputBoundary")
    fixed_args = require_field(request, "$.resultRequest", "fixedArgsPreview")
    require_string_array(fixed_args, "$.resultRequest.fixedArgsPreview", min_items=5)
    if fixed_args[:5] != ["hybrid", "source-intake", "result", "--format", "json"]:
        raise ShapeError("unexpected value at $.resultRequest.fixedArgsPreview")
    request_true_flags = ["summaryOnly", "inputRefOnly", "approvalRequired"]
    request_false_flags = [
        "approvalRequested",
        "approved",
    ...
    require_bool_fields(request, "$.resultRequest", request_true_flags + request_false_flags)
    for flag in request_true_flags:
        if request[flag] is not True:
            raise ShapeError(f"unexpected value at $.resultRequest.{flag}: expected true")
    for flag in request_false_flags:
        if request[flag] is not False:
            raise ShapeError(f"unexpected value at $.resultRequest.{flag}: expected false")

    result = expect_object(require_field(root, "$", "blockedResult"), "$.blockedResult")
    require_string_fields(
        result,
        "$.blockedResult",
        ["resultKind", "resultState", "approvalState", "intakeState", "disposition", "summary"],
    )
    expected_result = {
        "resultKind": "hybrid_source_intake_result",
        "resultState": "blocked_not_run",
        "approvalState": "not_approved",
        "intakeState": "not_started",
        "disposition": "blocked_until_explicit_hybrid_source_intake_approval",
    ...
    for field, expected in expected_result.items():
        if result[field] != expected:
            raise ShapeError(f"unexpected value at $.blockedResult.{field}: expected {expected}")
    result_true_flags = ["summaryOnly", "descriptorOnly", "approvalRequired"]
    result_false_flags = [
        "approved",
        "sourceIntakeAllowed",
        "sourceIntakeAttempted",
    ...
    require_bool_fields(result, "$.blockedResult", result_true_flags + result_false_flags)
    for flag in result_true_flags:
        if result[flag] is not True:
            raise ShapeError(f"unexpected value at $.blockedResult.{flag}: expected true")
    for flag in result_false_flags:
        if result[flag] is not False:
            raise ShapeError(f"unexpected value at $.blockedResult.{flag}: expected false")

    checklist = require_object_array(
        require_field(root, "$", "resultChecklist"),
        "$.resultChecklist",
        min_items=3,
    )
    checklist_true_flags = ["summaryOnly", "approvalRequired", "approvedSummaryRef"]
    ...
    for item in checklist:
        path = "$.resultChecklist[]"
        require_string_fields(item, path, ["checkId", "state", "sourceRef", "summary"])
        require_bool_fields(item, path, checklist_true_flags + checklist_false_flags)
        for flag in checklist_true_flags:
            if item[flag] is not True:
                raise ShapeError(f"unexpected value at $.resultChecklist[].{flag}: expected true")
        for flag in checklist_false_flags:
            if item[flag] is not False:
                raise ShapeError(f"unexpected value at $.resultChecklist[].{flag}: expected false")
    expected_checks = {
        "source_intake_approval_gate": "not_approved",
        "source_intake_not_attempted": "blocked_not_run",
        "no_result_payload_boundary": "blocked",
    }
    for check_id, state in expected_checks.items():
        if not any(item["checkId"] == check_id and item["state"] == state for item in checklist):
            raise ShapeError(f"missing result checklist item: {check_id}")

    policy = expect_object(require_field(root, "$", "intakeResultPolicy"), "$.intakeResultPolicy")
    require_string_fields(policy, "$.intakeResultPolicy", ["policyState", "resultState", "approvalState", "summary"])
    expected_policy = {
        "policyState": "blocked",
        "resultState": "blocked_not_run",
        "approvalState": "not_approved",
    ...
    for field, expected in expected_policy.items():
        if policy[field] != expected:
            raise ShapeError(f"unexpected value at $.intakeResultPolicy.{field}: expected {expected}")
    policy_true_flags = ["summaryOnly", "descriptorOnly", "approvalRequired", "auditRequired"]
    policy_false_flags = [
        "approved",
        "sourceIntakeAllowed",
    ...
    require_bool_fields(policy, "$.intakeResultPolicy", policy_true_flags + policy_false_flags)
    for flag in policy_true_flags:
        if policy[flag] is not True:
            raise ShapeError(f"unexpected value at $.intakeResultPolicy.{flag}: expected true")
    for flag in policy_false_flags:
        if policy[flag] is not False:
            raise ShapeError(f"unexpected value at $.intakeResultPolicy.{flag}: expected false")
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "hybridSourceIntakeResultFixtureOnly",
    ]
    safety_false_flags = [
        "approvalExecutorImplemented",
        "sourceIntakeApprovalImplemented",
...
def validate_sail_source_intake_result(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-source-intake-result.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "boundaryKind": "future_sail_source_intake_result_boundary",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    for ref_id in [
        ...
        if ref_id not in ref_ids:
            raise ShapeError(f"missing source boundary ref: {ref_id}")
    if not any(ref["refId"] == "sail_source_intake_approval" and ref.get("approved") is False for ref in refs):
        raise ShapeError("missing not-approved Sail source-intake approval reference")
    if not any(
    ...
    if request["requestKind"] != "planned_sail_source_intake_result":
        raise ShapeError("unexpected value at $.resultRequest.requestKind")
    if request["sourceReferenceKind"] != "approved-sail-source-intake-result-summaries":
        raise ShapeError("unexpected value at $.resultRequest.sourceReferenceKind")
    if request["outputBoundary"] != "pccx.lab.sail-source-intake-result.v0":
        raise ShapeError("unexpected value at $.resultRequest.outputBoundary")
    fixed_args = require_field(request, "$.resultRequest", "fixedArgsPreview")
    require_string_array(fixed_args, "$.resultRequest.fixedArgsPreview", min_items=5)
    if fixed_args[:5] != ["sail-model", "source-intake", "result", "--format", "json"]:
        raise ShapeError("unexpected value at $.resultRequest.fixedArgsPreview")
    request_true_flags = ["summaryOnly", "inputRefOnly", "approvalRequired"]
    request_false_flags = [
        "approvalRequested",
    ...
    expected_result = {
        "resultKind": "sail_source_intake_result",
        "resultState": "blocked_not_run",
        "approvalState": "not_approved",
        "intakeState": "not_started",
        "disposition": "blocked_until_explicit_sail_source_intake_approval",
    }
    for field, expected in expected_result.items():
        if result[field] != expected:
            raise ShapeError(f"unexpected value at $.blockedResult.{field}: expected {expected}")
    ...
    for required in [
        ...
        if required not in blocked_actions:
            raise ShapeError(f"missing blocked action: {required}")
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "sailSourceIntakeResultFixtureOnly",
    ]
    safety_false_flags = [
        "approvalExecutorImplemented",
        "sourceIntakeApprovalImplemented",
...
def validate_hybrid_source_intake_handoff(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-source-intake-handoff.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "boundaryKind": "future_hybrid_source_intake_handoff_summary",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    for ref_id in [
        ...
        if ref_id not in ref_ids:
            raise ShapeError(f"missing source boundary ref: {ref_id}")
    if not any(ref["refId"] == "hybrid_source_intake_result" and ref.get("approved") is False for ref in refs):
        raise ShapeError("missing blocked hybrid source-intake result reference")
    if not any(
        ref["refId"] == "hybrid_implementation_gap_matrix" and ref.get("sourceBoundaryGapRecorded") is True
        for ref in refs
    ):
    ...
    expected_request = {
        "requestKind": "planned_hybrid_source_intake_handoff",
        "sourceReferenceKind": "approved-hybrid-source-intake-handoff-summaries",
        "outputBoundary": "pccx.lab.hybrid-source-intake-handoff.v0",
    }
    ...
    if fixed_args[:5] != ["hybrid", "source-intake", "handoff", "--format", "json"]:
    ...
    expected_handoff = {
        "handoffKind": "hybrid_source_intake_handoff",
        "handoffState": "blocked_summary_only",
        "resultState": "blocked_not_run",
        "approvalState": "not_approved",
        "intakeState": "not_started",
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "hybridSourceIntakeHandoffFixtureOnly",
    ]
    safety_false_flags = [
        "approvalExecutorImplemented",
        "sourceIntakeApprovalImplemented",
...
def validate_sail_source_intake_status_summary(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.sail-source-intake-status-summary.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_root_values = {
        ...
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "boundaryKind": "future_sail_source_intake_status_summary",
    }
    for field, expected in expected_root_values.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    for ref_id in [
        ...
        if ref_id not in ref_ids:
            raise ShapeError(f"missing source boundary ref: {ref_id}")
    for ref_id in ["sail_source_intake_approval", "sail_source_intake_result", "sail_source_intake_handoff"]:
        if not any(ref["refId"] == ref_id and ref.get("approved") is False for ref in refs):
            raise ShapeError(f"missing blocked Sail source-intake status reference: {ref_id}")
    if not any(
        ref["refId"] == "sail_implementation_gap_matrix" and ref.get("sourceBoundaryGapRecorded") is True
        for ref in refs
    ):
    ...
    expected_summary_values = {
        "summaryKind": "planned_sail_source_intake_status",
        "sourceReferenceKind": "approved-sail-source-intake-status-summaries",
        "selectedTrack": "sail_source_intake",
    ...
    for card_id in ["source_boundary", "approval_gate", "result_gate", "handoff_gate", "source_access_gate"]:
        if card_id not in card_ids:
            raise ShapeError(f"missing Sail source-intake status card: {card_id}")
    ...
    if fixed_args[:5] != ["sail-model", "source-intake", "status", "--format", "json"]:
    ...
    for step_id in ["source_boundary_status", "approval_status", "result_status", "handoff_status"]:
        if step_id not in step_ids:
            raise ShapeError(f"missing Sail source-intake status-summary flow step: {step_id}")
    require_string_array(
        require_field(flow, "$.summaryFlow", "blockedReasonRefs"),
        "$.summaryFlow.blockedReasonRefs",
        min_items=1,
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "sailSourceIntakeStatusSummaryFixtureOnly",
    ]
    safety_false_flags = [
        ...
        "modelGenerationImplemented",
        "modelExecutionImplemented",
        "refinementCheckImplemented",
        "formalProofImplemented",
        "simulatorExecutionImplemented",
...
def validate_hybrid_interface_boundary(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-interface-boundary.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    for ref in refs:
        require_string_fields(ref, "$.sourceBoundaryRefs[]", ["refId", "schemaVersion", "examplePath", "state"])
    if not any(
        ref["refId"] == "hybrid_strategy_plan"
        and ref.get("interfaceSource") is True
        and ref.get("sourceReadAllowed") is False
        and ref.get("executionAllowed") is False
        for ref in refs
    ):
        raise ShapeError("missing hybrid_strategy_plan interface source reference")
    if not any(
        ref["refId"] == "workflow_descriptors"
        and ref.get("summaryReferenceAllowed") is True
        and ref.get("commandExecutionAllowed") is False
        ...
        raise ShapeError("missing workflow_descriptors hybrid interface reference")
    if not any(
        ref["refId"] == "workflow_results"
        and ref.get("resultSummaryAvailable") is True
        and ref.get("payloadIncluded") is False
    ...
    output_false_flags = [
        ...
        "compiledArtifactIncluded",
        "runtimePlanIncluded",
        "verificationResultIncluded",
        "simulatorOutputIncluded",
        "hardwareControlOutputIncluded",
        "traceContentIncluded",
        "reportContentIncluded",
        "stdoutIncluded",
        "stderrIncluded",
...
def validate_hybrid_review_packet(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-review-packet.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "adapterState": "not_implemented",
        "defaultMode": "read_only",
        "packetKind": "summary_only_hybrid_review_packet",
        "automationPath": "cli_core_first",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
    ...
    for item in inputs:
        ...
        for flag in input_false_flags:
            if item[flag] is not False:
                raise ShapeError(f"unexpected value at $.reviewInputs[].{flag}: expected false")
        fields = require_object_array(
            require_field(item, path, "fieldDescriptors"),
            child(path, "fieldDescriptors"),
            min_items=1,
        )
        for field in fields:
    ...
    if policy["state"] != "planned":
        raise ShapeError("unexpected value at $.reviewPolicy.state: expected planned")
    if policy["packetKind"] != "summary_only_hybrid_review_packet":
        raise ShapeError("unexpected value at $.reviewPolicy.packetKind: expected summary_only_hybrid_review_packet")
    expect_integer(require_field(policy, "$.reviewPolicy", "maxSections"), "$.reviewPolicy.maxSections")
    policy_true_flags = ["summaryOnly", "approvalRequired", "auditRequired"]
    policy_false_flags = [
        "cppSourceReadAllowed",
    ...
    for array_name, min_items in [("riskRows", 1), ("checklistItems", 1), ("validationLines", 1)]:
        rows = require_object_array(
            require_field(packet, "$.sampleReviewPacket", array_name),
            child("$.sampleReviewPacket", array_name),
            min_items=min_items,
        )
        for row in rows:
            path = f"$.sampleReviewPacket.{array_name}[]"
    ...
    safety = expect_object(require_field(root, "$", "safetyFlags"), "$.safetyFlags")
    safety_true_flags = ["dataOnly", "descriptorOnly", "readOnly", "hybridReviewPacketFixtureOnly", "summaryOnly", "cliCoreFirst"]
    safety_false_flags = [
        "cppSourceReaderImplemented",
        "systemVerilogSourceReaderImplemented",
        "scriptSourceReaderImplemented",
...
def validate_hybrid_evidence_manifest(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-evidence-manifest.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "adapterState": "not_implemented",
        "defaultMode": "read_only",
        "manifestKind": "approved_hybrid_summary_evidence_refs",
        "automationPath": "cli_core_first",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
    ...
    if policy["state"] != "planned":
        raise ShapeError("unexpected value at $.manifestPolicy.state: expected planned")
    if policy["manifestKind"] != "summary_only_hybrid_evidence_refs":
        raise ShapeError("unexpected value at $.manifestPolicy.manifestKind: expected summary_only_hybrid_evidence_refs")
    expect_integer(require_field(policy, "$.manifestPolicy", "maxEvidenceRefs"), "$.manifestPolicy.maxEvidenceRefs")
    policy_true_flags = ["summaryOnly", "approvalRequired", "auditRequired"]
    policy_false_flags = [
        "cppSourceReadAllowed",
    ...
    safety = expect_object(require_field(root, "$", "safetyFlags"), "$.safetyFlags")
    safety_true_flags = ["dataOnly", "descriptorOnly", "readOnly", "hybridEvidenceManifestFixtureOnly", "summaryOnly", "cliCoreFirst"]
    safety_false_flags = [
        "cppSourceReaderImplemented",
        "systemVerilogSourceReaderImplemented",
        "scriptSourceReaderImplemented",
...
def validate_hybrid_evidence_detail(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-evidence-detail.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    for ref in refs:
        require_string_fields(ref, "$.sourceBoundaryRefs[]", ["refId", "schemaVersion", "examplePath", "state"])
    if not any(
        ref["refId"] == "hybrid_evidence_manifest"
        and ref.get("evidenceManifestAvailable") is True
        and ref.get("evidenceDetailAvailable") is True
        and ref.get("summaryOnly") is True
        and ref.get("sourceReaderAllowed") is False
        and ref.get("grammarReaderAllowed") is False
        and ref.get("artifactReaderAllowed") is False
        and ref.get("reportReaderAllowed") is False
        and ref.get("reportWriterAllowed") is False
        and ref.get("commandExecutionAllowed") is False
        ...
        raise ShapeError("missing hybrid_evidence_manifest disabled source reference")
    if not any(
        ref["refId"] == "hybrid_review_packet"
        and ref.get("summaryOnly") is True
        and ref.get("sourceReadAllowed") is False
        and ref.get("grammarReadAllowed") is False
        and ref.get("commandExecutionAllowed") is False
        and ref.get("reportWriterAllowed") is False
        and ref.get("artifactWriteAllowed") is False
        and ref.get("repositoryMutationAllowed") is False
        ...
        raise ShapeError("missing hybrid_review_packet summary-only source reference")
    if not any(
        ref["refId"] == "hybrid_interface_boundary"
        and ref.get("summaryOnly") is True
        and ref.get("sourceReadAllowed") is False
        and ref.get("grammarReadAllowed") is False
        and ref.get("executionAllowed") is False
        ...
        raise ShapeError("missing hybrid_interface_boundary descriptor source reference")
    ...
    if request["requestKind"] != "planned_hybrid_evidence_detail":
        raise ShapeError(
            "unexpected value at $.evidenceDetailRequest.requestKind: expected planned_hybrid_evidence_detail"
        )
    if request["selectedEvidenceId"] != "hybrid_strategy_summary_evidence":
        raise ShapeError(
            "unexpected value at $.evidenceDetailRequest.selectedEvidenceId: "
            "expected hybrid_strategy_summary_evidence"
        )
    require_string_array(
        require_field(request, "$.evidenceDetailRequest", "fixedArgsPreview"),
        "$.evidenceDetailRequest.fixedArgsPreview",
    ...
    safety = expect_object(require_field(root, "$", "safetyFlags"), "$.safetyFlags")
    safety_true_flags = ["dataOnly", "descriptorOnly", "readOnly", "summaryOnly", "hybridEvidenceDetailFixtureOnly"]
    safety_false_flags = [
        "cppSourceReaderImplemented",
        "systemVerilogSourceReaderImplemented",
        "scriptSourceReaderImplemented",
...
def validate_hybrid_implementation_readiness(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-implementation-readiness.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "gateKind": "future_hybrid_implementation_readiness_gate",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    if request["requestKind"] != "planned_hybrid_implementation_readiness":
        raise ShapeError("unexpected value at $.readinessRequest.requestKind")
    if request["outputBoundary"] != "pccx.lab.hybrid-implementation-readiness.v0":
        raise ShapeError("unexpected value at $.readinessRequest.outputBoundary")
    fixed_args = require_field(request, "$.readinessRequest", "fixedArgsPreview")
    require_string_array(fixed_args, "$.readinessRequest.fixedArgsPreview", min_items=4)
    if fixed_args[:4] != ["hybrid", "readiness", "--format", "json"]:
        raise ShapeError("unexpected value at $.readinessRequest.fixedArgsPreview")
    request_true_flags = ["summaryOnly", "inputRefOnly"]
    request_false_flags = [
        "approvalRequired",
    ...
    require_string_fields(
        result,
        "$.readinessResult",
        ["resultState", "implementationReadiness", "cliCoreGate", "guiGate", "summary"],
    )
    if result["resultState"] != "metadata_only":
        raise ShapeError("unexpected value at $.readinessResult.resultState: expected metadata_only")
    if result["implementationReadiness"] != "not_ready_for_execution":
        raise ShapeError("unexpected value at $.readinessResult.implementationReadiness")
    result_true_flags = ["summaryOnly"]
    result_false_flags = [
        "readyForSourceIntake",
        "readyForParser",
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "hybridImplementationReadinessFixtureOnly",
    ]
    safety_false_flags = [
        "cppSourceReaderImplemented",
        "systemVerilogSourceReaderImplemented",
...
def validate_hybrid_implementation_gap_matrix(value: Any) -> None:
    root = expect_object(value, "$")
    require_schema(root, "$", "pccx.lab.hybrid-implementation-gap-matrix.v0")
    require_string_fields(
        root,
        "$",
        [
    ...
    expected_states = {
        ...
        "defaultMode": "read_only",
        "hostMode": "cli_core_first_gui_second",
        "matrixKind": "future_hybrid_implementation_gap_matrix",
    }
    for field, expected in expected_states.items():
        if root[field] != expected:
            raise ShapeError(f"unexpected value at $.{field}: expected {expected}")
    ...
    if request["requestKind"] != "planned_hybrid_implementation_gap_matrix":
        raise ShapeError("unexpected value at $.gapRequest.requestKind")
    if request["outputBoundary"] != "pccx.lab.hybrid-implementation-gap-matrix.v0":
        raise ShapeError("unexpected value at $.gapRequest.outputBoundary")
    fixed_args = require_field(request, "$.gapRequest", "fixedArgsPreview")
    require_string_array(fixed_args, "$.gapRequest.fixedArgsPreview", min_items=4)
    if fixed_args[:4] != ["hybrid", "gap-matrix", "--format", "json"]:
        raise ShapeError("unexpected value at $.gapRequest.fixedArgsPreview")
    request_true_flags = ["summaryOnly", "inputRefOnly"]
    request_false_flags = [
        "approvalRequired",
    ...
    safety_true_flags = [
        ...
        "readOnly",
        "summaryOnly",
        "hybridImplementationGapMatrixFixtureOnly",
    ]
    safety_false_flags = [
        "cppSourceReaderImplemented",
        "systemVerilogSourceReaderImplemented",
...
SPECS = [
    ...
    BoundarySpec("sail-interface-boundary", "docs/examples/sail-interface-boundary.example.json", validate_sail_interface_boundary),
    BoundarySpec("sail-review-packet", "docs/examples/sail-review-packet.example.json", validate_sail_review_packet),
    BoundarySpec("sail-evidence-manifest", "docs/examples/sail-evidence-manifest.example.json", validate_sail_evidence_manifest),
    ...
    BoundarySpec("sail-source-intake-status-summary", "docs/examples/sail-source-intake-status-summary.example.json", validate_sail_source_intake_status_summary),
    ...
    BoundarySpec("hybrid-evidence-manifest", "docs/examples/hybrid-evidence-manifest.example.json", validate_hybrid_evidence_manifest),
    ...
    BoundarySpec("mcp-sample-status-summary", "docs/examples/mcp-sample-status-summary.example.json", validate_mcp_sample_status_summary),
    ...
    BoundarySpec("plugin-sample-status-summary", "docs/examples/plugin-sample-status-summary.example.json", validate_plugin_sample_status_summary),
